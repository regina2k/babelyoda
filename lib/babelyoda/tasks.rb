  ...
  Babelyoda::Rake.spec do |spec|
    ...
    task :extract_strings do
      spec.scm.transaction("[Babelyoda] Extract strings from sources") do 
        Babelyoda.logger.info "Extracting strings from sources..."
        ...
        Babelyoda::Genstrings.run(spec.source_files, dev_lang) do |keyset|
          ...
          old_strings.save!
          Babelyoda.logger.debug "#{old_strings_filename}: #{old_strings.keys.size} keys"
        end
      end
    end
    ...
    task :extract_xib_strings do
      spec.scm.transaction("[Babelyoda] Extract strings from XIBs") do 
        Babelyoda.logger.info "Extracting .strings from XIBs..."
        spec.xib_files.each do |xib_filename|
          ...
          keyset = xib.strings
          unless keyset.empty?
            Babelyoda.logger.debug "#{xib_filename} => #{xib.strings_filename}"
    ...
    task :drop_empty_strings do
      spec.scm.transaction("[Babelyoda] Drop empty .strings files") do 
        Babelyoda.logger.info "Dropping empty .strings files..."
        files_to_drop = []
        ...
        files_to_drop.each do |filename|
          Babelyoda.logger.info "REMOVED empty file: #{filename}"
    ...
    task :create_keysets => [:extract, :drop_empty_strings] do
      Babelyoda.logger.info "Creating remote keysets for local keysets..."
      ...
      spec.strings_files.each do |filename|
        ...
        if remote_keyset_names.include?(keyset_name)
          Babelyoda.logger.debug "Tanker: An existing keyset found: #{keyset_name}"
          next 
        end
        strings = Babelyoda::Strings.new(filename, spec.development_language).read
        unless strings.empty?
          spec.engine.create(keyset_name)
          Babelyoda.logger.debug "Tanker: Created NEW keyset: #{keyset_name}"
        end
      end
    end
    ...
    task :drop_orphan_keysets => :create_keysets do
      Babelyoda.logger.info "Dropping orphan keysets..."
      ...
      spec.engine.list.each do |remote_keyset_name|
        unless local_keysets.include?(remote_keyset_name)
          Babelyoda.logger.debug "Dropping keyset: #{remote_keyset_name}"
          spec.engine.drop_keyset!(remote_keyset_name)
          count += 1
        end
      end
      Babelyoda.logger.info "Dropped keysets: #{count}" if count > 0
    end
    
    desc "Drops remote keys not found in local keysets"
    task :drop_orphan_keys => :create_keysets do
      Babelyoda.logger.info "Dropping orphan keys..."
      spec.strings_files.each do |filename|
        strings = Babelyoda::Strings.new(filename, spec.development_language).read!
        Babelyoda.logger.debug "Processing keyset: #{strings.name}"
        ...
        remote_keyset.keys.delete_if do |key, value|
          unless strings.keys.has_key?(key)
            Babelyoda.logger.debug "Found orphan key: #{key}"
            true
          else
            false
          end
        ...
        unless remote_keyset.empty?
          Babelyoda.logger.debug "Keys removed: #{original_keys_size - remote_keyset.keys.size}, keyset REPLACED."
          spec.engine.replace(remote_keyset)
        else
          Babelyoda.logger.debug "All keys removed: keyset DELETED."
    ...
    task :push_keys => :create_keysets do
      ...
      Babelyoda.logger.info "Pushing local keys for '#{langs.join(', ')}' to the remote..."
      spec.strings_files.each do |filename|
        local_keyset = Babelyoda::Strings.new(filename, spec.development_language).read!
        Babelyoda.logger.debug "Processing keyset: #{local_keyset.name}"
        ...
        if result[:new] > 0 || result[:updated] > 0
          ...
          Babelyoda.logger.debug "New keys: #{result[:new]} Updated keys: #{result[:updated]}"
        end
      end
    end
    ...
    task :fetch_strings do
      spec.scm.transaction("[Babelyoda] Merge in remote translations") do 
        Babelyoda.logger.info "Fetching remote translations..."
        spec.strings_files.each do |filename|
          keyset_name = Babelyoda::Keyset.keyset_name(filename)
          ...
          spec.all_languages.each do |language|
            ...
            Babelyoda.logger.debug "#{keyset_filename}"
          end
        end
      end
    end
    ...
    task :update_xib_hashes do    
      spec.scm.transaction("[Babelyoda] Update XIB SHA1 version refs") do 
        Babelyoda.logger.info "Update XIB SHA1 version refs..."
    ...
    task :localize_xibs_wo_updating_xib_hashes do
      spec.scm.transaction("[Babelyoda] Localize XIB files") do 
        Babelyoda.logger.info "Translating XIB files..."
        spec.xib_files.each do |filename|
          ...
          if xib.localizable?
            ...
            Babelyoda.logger.warn "#{filename} has no localizable resources. No localization needed."
          end
        end
      end      
    end
